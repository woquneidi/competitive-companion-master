...
export class CodeforcesProblemParser extends Parser {
  ...
  public async parse(url: string, html: string): Promise<Sendable> {
    ...
    if (url.includes('/problemsets/acmsguru')) {
      ...
      return this.parseMainProblem(html, url, task);
    }
  }

  private parseMainProblem(html: string, url: string, task: TaskBuilder): Sendable {
    const elem = htmlToElement(html);
    ...
    if (url.includes('/edu/')) {
      ...
      breadcrumbs.pop();
      originalGroup = breadcrumbs.join(' - ');
      task.setGroup('Codeforces');
    } else {
      const contestType = url.includes('/gym/') ? 'gym' : 'contest';
      const titleElem = elem.querySelector(`.rtable > tbody > tr > th > a[href*='${contestType}']`);

      if (titleElem !== null) {
        originalGroup = titleElem.textContent.trim();
      }
      task.setGroup('Codeforces');
    }
    (task as any).contestname = originalGroup;
    ...
    (result as any).contestname = originalGroup;
    return result;
  }
